...
contributors:
  - role: wrote
    users: [mattaereal, zedt3ster, fredriksvantes, fantasy]
  - role: reviewed
    users: [mattaereal]
---
...
However, depending on the method used for obtaining this phone number, additional risk or privacy concerns may be 
introduced when accessing or maintaining it. For example, using a VoIP number requires securing access to the VoIP 
account; using a collectible number requires securing the wallet used to store the number.

#### Using a VoIP Number

Telegram restricts many VoIP providers, but services such as [Google Voice](https://voice.google.com/),
[Burner](https://www.burnerapp.com/), or [JMP Chat](https://jmp.chat/) might work. Purchase a burner number 
solely for Telegram if you prefer additional anonymity. Using a VoIP number to create an account may incur an 
additional Telegram-enforced fee before sending a verification SMS OTP.

#### Using a Collectible Number

In December 2022, Telegram introduced support for [collectible numbers](https://telegram.org/blog/ultimate-privacy-topics-2-0#sign-up-without-a-sim-card)
(previously known as "anonymous" numbers) purchased through its [TON](https://ton.org/) blockchain infrastructure.
You can also check out [Fragment](https://fragment.com/numbers) for such options.
...
- **Enhance Privacy with a VPN:** *Advanced tip:* Set up a proxy, such as Tor, or VPN to hide your IP address while 
